...
def load_states(screen, wall_states, block_states,
                    ball_states, paddle_states):
    max_x = max([k[0] for k in screen.keys()])
    max_y = max([k[1] for k in screen.keys()])
    wall_points = []
    block_points = []
    ...
    for y in range(max_y+1):
        for x in range(max_x+1):
            ...
            if tile_id == '0':
                pass # nothing here
            elif tile_id == '1' and len(wall_states) < 1:
                wall_points.append((x, y))
            elif tile_id == '2':
                block_points.append((x, y))
            elif tile_id == '3':
                paddle_point.append((x, y))
            elif tile_id == '4':
                ball_point.append((x, y))  # ball
    if len(wall_states) == 0:
        wall_states.append(np.array(wall_points))
    else:
        wall_states.append(np.copy(wall_states[0]))
    block_states.append(np.array(block_points))
    ball_states.append(np.array(ball_point))
    paddle_states.append(np.array(paddle_point))
...
def part2(ls):
    ...
    while mac_state != -1:
        ...
        process_output(outputs, screen, score, ball, ball_dir,
                       paddle, walls, old_ball)
        load_states(screen, wall_states, block_states,
                    ball_states, paddle_states)
    ...
    # do the animation from the saved states
    ...
    # first draw
    plot_walls = ax.scatter(wall_states[0][:,0], wall_states[0][:,1], 
                     marker='s', s=180, color='gray')
    plot_blocks = ax.scatter(block_states[0][:,0], block_states[0][:,1], 
                     marker='s', s=180, color='blue')
    plot_ball = ax.scatter(ball_states[0], ball_states[1], 
                    marker='o', s=180, color='red')
    plot_paddle = ax.scatter(paddle_states[0], paddle_states[1], 
                    marker='_', s=360, linewidths=3, color='green')
    def update(frame):
        plot_walls.set_offsets(wall_states[frame])
        if len(block_states[frame]) > 0:
            plot_blocks.set_offsets(block_states[frame])
        plot_ball.set_offsets(ball_states[frame])
        plot_paddle.set_offsets(paddle_states[frame])
        return(plot_walls, plot_blocks, plot_ball, plot_paddle)
    
    
    ani = FuncAnimation(fig, update, frames=len(ball_states)-1, interval=16, blit=True)
    ani.save("d13.mp4", fps=30)
    #plt.show()
